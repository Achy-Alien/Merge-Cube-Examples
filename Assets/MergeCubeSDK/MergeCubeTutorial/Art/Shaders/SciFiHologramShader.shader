...
Shader "Zololgo/Sci-Fi Hologram" 
{
	Properties
	{
		...
		_OffsetScrollSpeedX("X Scroll Speed", Range(-100, 100)) = -20
		_OffsetScrollSpeedY("Y Scroll Speed", Range(-100, 100)) = -20
	...
	SubShader
	{
		Tags { "RenderType"="Transparent" "Queue"="OverLay+5"}
		LOD 100
		...
		Pass
		{
			...
			fixed _OffsetScrollSpeedX, _OffsetScrollSpeedY;
			...
			v2f vert (appdata v)
			{
				...
				flicker_uv.y = worldPos.y;


				//offset code
				o.scanlines_uv = flicker_uv * _Scanlines_ST.xy + frac(_Time.x * _OffsetScrollSpeedY); // + screenPosOrigin.w; // + screenPos.w); // + _Scanlines_ST.w;
			...
			{
				fixed4 main = tex2D(_MainTex, i.main_uv) * _Brightness;
				fixed4 rim = i.color;
